...
const InstractorDetail = () => {
  ...
  useEffect(() => {
    ;(async function () {
      const data = await axios.get(`/instructor/${params.id}`)
  ...
  const submit = async (_id) => {
    const data = await axios.delete(`/deleteinstractor/${_id}`)
    // alert(_id)
    console.log(data)
      ...
      {data && (
        <div className='mt-3 space-y-6 drop-shadow-lg rounded-lg'>
          ...
          <div>
            ...
            <button
              className='rounded mx-3 border-red-700 border-[1px] py-[4px] px-10'
